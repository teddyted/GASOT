function CrosswordObject(searchBoxId, selectListId, selectedListParam,displayCrossword, gameboard, collectionJSONParam){
    ...
    function init(){
        loadFilter();
        displayList();
        $('body').append('<iframe id="crosswordHelperPrintFrame" style="width: 0px;height:0px;visibility: visible"></iframe>');
    }
    ...
    function printImage(){
        ...
        var objFrame =  $('#crosswordHelperPrintFrame');

        if (!objFrame.length){
            return;
        }

        var content = $('#' + displayCrossword).html();
        objFrame.contents().find('body').html('<div id="'+displayCrossword+'">'+content + '</div>');
        ...
        $('#crosswordHelperPrintFrame')[0].focus();
    ...
    function _checkObj(index){
        ...
        if(size>=10){
            ...
            if (size==10) alert('Sorry, you have a limited space for ten words.');
            return rtn;
    ...
    function displayHeader(){
        ...
        return '<h1 id="displayCrosswordHeadingText">' + ( scrambleAndWords ? headingSpellingOnly : headingSpellingWords )  + '</h1>';

    }

    function displayCrosswordImages(){
        ...
        for(var i = 0; i < words.length; i++ ){
            ...
            params.push({word: word, clue: 1});
        }
        gameboard.clearBoard();
        gameboard.generateBoard(params);
    ...
    this.displayFingerSpellingOnly = function(){
        ...
        $("#displayCrosswordHeadingText").html(headingSpellingOnly);
        $('.spellingwords').hide();
        $('.spellingonly').show();
    ...
    this.displayFingerSpellingWords = function(){
        ...
        $("#displayCrosswordHeadingText").html(headingSpellingWords);
        $('.spellingwords').show();
        $('.spellingonly').hide();
